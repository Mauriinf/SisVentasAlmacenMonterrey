...
{
    public partial class FrmVistaEditarProducto_Venta : Form
    {
        public FrmVistaEditarProducto_Venta()
        {
            InitializeComponent();
        }
        //Método Mostrar
        ...
        {
            this.dataListado.DataSource = NVenta.MostrarProducto_Venta_NombreNombre(this.txtBuscar.Text);
        ...
        private void FrmVistaEditarProducto_Venta_Load(object sender, EventArgs e)
        {
            this.Mostrar();
        }
        ...
        {
            FrmEditarDetalleVentas form = FrmEditarDetalleVentas.GetInstancia();
            string par1, par2;
            string par3,par4;
            par1 = Convert.ToString(this.dataListado.CurrentRow.Cells["iddetalle_ingreso"].Value);
            par4 = Convert.ToString(this.dataListado.CurrentRow.Cells["stock_actual"].Value);
            par2 = Convert.ToString(this.dataListado.CurrentRow.Cells["ID"].Value);
            par3 = Convert.ToString(this.dataListado.CurrentRow.Cells["nombre"].Value);
            form.setProducto(par1, par2, par3,par4);
            this.Hide();
        }
        ...
        private void dataListado_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == (Keys.Enter))
            {
                ...
                par1 = Convert.ToString(this.dataListado.Rows[dataListado.CurrentRow.Index].Cells["iddetalle_ingreso"].Value);
                par4 = Convert.ToString(this.dataListado.Rows[dataListado.CurrentRow.Index].Cells["stock_actual"].Value);
                par2 = Convert.ToString(this.dataListado.Rows[dataListado.CurrentRow.Index].Cells["ID"].Value);
                par3 = Convert.ToString(this.dataListado.Rows[dataListado.CurrentRow.Index].Cells["nombre"].Value);
                form.setProducto(par1, par2, par3, par4);
                this.Hide();
            }
        }
